...
| 🏎️ 飞车 3D · 极速漂移 | 竞速漂移 racing | `qq-speed/` | 17 个 JS 模块 + HTML | ~770 KB | [claude-opus-5-5-qqfeiche3d.pages.dev](https://claude-opus-5-5-qqfeiche3d.pages.dev/) |
...
原创 3D 漂移竞速：Shift 漂移、Ctrl 氮气、小喷与双喷、复位键，首版内置四张赛道：霓虹都市（11 处弯 + 发卡弯）、爱琴海岸（爬坡连续弯）、沙海神殿（直道末端跳台）、冰雪峡谷（8 字形立交）。赛道形状先用脚本生成俯视图检查弯道与交叉点，再进入正式开发。
...
- **新赛道「原始森林」** — 1.86 km 技术型林道（半径 41~90 m）：蛇形林道 → 下坡过溪谷木桥 → 爬坡发卡弯 → 坡顶小跳 → 发光年轮的树洞隧道 → 绕 88 m 巨木的 140° 长弯；瀑布与水潭、发光蘑菇、符文石柱、萤火虫，专属 BGM
...
- **3 款概念跑车**（按原型图建模，属性条即设计数值）
  - **隼影 APEX-01**（赛道跑车，12000 金币）：终极漂移车，加速 / 操控 / 漂移基础值直接顶满属性条（极速 86 · 加速 100 · 操控 100 · 漂移 100 · 氮气 90）；短轴距超宽轮距、水滴座舱、四个独立轮拱、一字眉青色大灯、双层尾翼、U 形尾灯
  - **裂空 VELOCITY-02**（楔形超跑，8800 金币）：极速特化（96 / 78 / 68 / 75 / 88）；修长楔形车身、箭头车鼻、黑色中脊、平贴尾翼与双尾鳍、琥珀贯穿尾灯与圆形氮气口
  - **雷脉 PULSE-03**（GT 肌肉车，9600 金币）：加速特化（82 / 96 / 82 / 84 / 90）；方正车身、阶梯宽轮拱、双方块大灯、中位单尾翼、叠放方形氮气口
- **撤回赛博太空风格**：去掉各赛道的带环行星、卫星与星空，界面 / 仪表盘 / 小地图 / 车库恢复第三轮的配色；原始森林改为晴天
...
> 使用原型图，增加三款车型。并恢复到上一版不是赛博太空的场景。其中一款车要求操控数值是数值极限，加速也是极限，是一款非常适合漂移的终极赛车。
...
| 飞车 3D · 极速漂移 | <https://claude-opus-5-5-qqfeiche3d.pages.dev/> |
...
- 游戏为能力测试用途的作品，与《穿越火线》原厂无关；飞车 3D 为原创竞速，卡丁车与赛道均为原创造型
